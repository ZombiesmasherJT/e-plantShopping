...
function ProductList({ onHomeClick }) {
  ...
  // 🔢 Task 4: calculate total quantity of items in cart
  ...
  const handleAddToCart = (product) => {
    // Step 1: Add product to global Redux store
    dispatch(addItem(product));

    // Step 2: Update local state so this product shows as added
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: true, // marks that one product as added
    }));
  };
    ...
    <div>
      ...
      {!showCart ? (
        <div className="product-grid">
          {/* Outer map – loop through each plant category */}
          {plantsArray.map((category, index) => (
            <div key={index}>
              {/* Category title */}
              <h2>{category.category}</h2>

              {/* Inner map – loop through plants inside this category */}
